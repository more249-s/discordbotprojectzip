...
class AsuraProvider(BaseProvider):
    ...
    async def get_images(self, url):
        try:
            ...
            # 2. استخراج الصور المشفرة في JSON أو كود JavaScript (النظام الجديد لـ Asura)
            if not images:
                # البحث عن روابط الصور المباشرة في النص
                patterns = [
