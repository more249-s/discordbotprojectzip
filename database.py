...
async def is_transaction_new(tx_id):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT 1 FROM binance_transactions WHERE tx_id = ?", (tx_id,)
        ) as cursor:
...
async def get_active_alerts():
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT alert_id, user_id, symbol, target_price, condition "
...
async def deactivate_alert(alert_id):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE price_alerts SET is_active = 0 WHERE alert_id = ?", (alert_id,)
        )
        await db.commit()
